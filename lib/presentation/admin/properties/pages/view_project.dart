...
class _ViewProjectState extends State<ViewProject> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('projects').orderBy('order_id').snapshots(),
          ...
          if(snapshot.hasData){
            ...
            return ReorderableListView.builder(
              ...
              onReorder: (oldIndex, newIndex)async{
                ...
                await FirebaseFirestore.instance.collection('projects').doc(projectsOld).update({
                  'order_id' : newOrderId,
                });
                await FirebaseFirestore.instance.collection('projects').doc(projectsNew).update({
                  'order_id' : oldOrderId,
                });
              ...
              itemBuilder: (context,index){
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      width: 2.w,
                      color: settings!.featuredProjects!.contains(data[index]['id']) ? AppColors.kRedColor : Colors.transparent,
                    )
                  ),
                  key: Key('era$index'),
                  child: ListTile(
                    ...
                    trailing: Wrap(
                      ...
                      children: [
                        TextButton(
                          onPressed: ()async{
                            if(settings!.featuredProjects!.contains(data[index]['id'])){
                              settings!.featuredProjects!.removeAt(settings!.featuredProjects!.indexOf(data[index]['id']));
                              await settings!.update();
                            }else{
                              settings!.featuredProjects!.add(data[index]['id']);
                              await settings!.update();
                            }
                          ...
                          child: EraText(
                            text: "${settings!.featuredProjects!.contains(data[index]['id']) ? "Remove from" : "Add to"} Featured",
                            color: Colors.black,
                          ),
                        ),
                        IconButton(
