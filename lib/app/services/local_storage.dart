...
class LocalStorageService extends GetxService {
  ...
  set images(value) {
    if (value != null) {
      _sharedPreferences?.setString(
          _Key.downloadedImage.toString(), jsonEncode(value));
    }
  }

  Map<String, dynamic>? get images {
    if (_sharedPreferences?.getString(_Key.downloadedImage.toString()) !=
        null) {
      return jsonDecode(
          _sharedPreferences?.getString(_Key.downloadedImage.toString()));
    } else {
      return null;
    }
  }
  ...
  set settings(Settings? value) {
    if (value != null) {
      _sharedPreferences?.setString(
          _Key.settings.toString(), jsonEncode(value.toMap()));
    }
  }
